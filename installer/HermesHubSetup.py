...
def verify_dependencies(venv_python: Path) -> bool:
    """Verify that required packages can be imported without error."""
    code = "import yaml; import psutil; import fastapi; import uvicorn; print('OK')"
    try:
        res = subprocess.run(
...
def install_dependencies(venv_python: Path) -> bool:
    """Install required packages into Hermes venv."""
...
def run_installation(silent: bool = False):
    ...
    print("\n[2/5] Проверка и установка зависимостей (FastAPI, uvicorn, psutil, pyyaml)...")
    if not verify_dependencies(venv_python):
        ...
        if not ok or not verify_dependencies(venv_python):
            print("\n" + "!" * 60)
            print(" [ОШИБКА УСТАНОВКИ] Не удалось установить зависимости (FastAPI / uvicorn / psutil / pyyaml)!")
